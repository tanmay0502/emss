...
export default function OrderList() {
    ...
    const mapOrderStatus = {
        ...
        "SD": "Sender district supply information provided",
    ...
    function ExpandingText(props) {
        const { text, shortText, expandOnHover = true } = props;
        ...
        return (
            <div
                ...
                onClick={handleClick}
                className={`text-gray-700 cursor-pointer ${isExpanded ? 'px-0': 'px-16 text-clip'
                    }`}
            >
        ...
        if (data) {
                    ...
                    if (!subordid.includes(val['orderid'])) {
                        ...
                        data.map(function (v) {
                            if (v["referenceorderid"].split(':__').length > 1 && v['orderid'] != val['orderid']) {
                                if (val["referenceorderid"].split(':_')[0] === v["referenceorderid"].split(':_')[0]) {
            ...
            for (let i = 0; i < pp.length; i++) {
                pp[i]["print"] = (<button key={i} target='_blank' onClick={(e) => {
                    e.stopPropagation();
                    generatePDF(pp[i]["displayID"])
                }} className="text-white">Show Pdf</button>);
                pp[i]["type"] = (<button key={i} target='_blank'className="text-white" style={{background: dict[pp[i]["type"]]}}>
                {(pp[i]["type"])}
            </button>);
                pp[i]["orderstatus"] = (<ExpandingText text= {mapOrderStatus[pp[i]["orderstatus"]]}
                shortText={pp[i]["orderstatus"] }
                expandOnHover={true}>
    ...
    const sortTableData = (sortBy, reverse = false) => {
        const sorted = tableData.sort((a, b) => {
            ...
            // console.log(a.type.localeCompare(b.type));
            // console.log(b.type.localeCompare(a.type));
            if (sortBy === "Type") {
        ...
        if (sortBy === "Default") {
            ...
            for (let i = 0; i < sorted.length; i++) {
                sorted[i]["print"] = (<button key={i} target='_blank' onClick={(e) => {
                    e.stopPropagation();
                    generatePDF(sortedTableData[i]["displayID"])
                }} className="text-white">Show Pdf</button>);
            }
                ...
                {isDetail == 0 ?
                    <div className={styles.Scroll}>
                        ...
                        <DynamicDataTable
                            ...
                            fieldMap={
                                {
                                    'displayID': (<div onClick={() => { sortTableData("OrderID") }} className="flex cursor-pointer justify-center text-orange-600">Order ID</div>),
                                    'creatoruserid': (<div onClick={() => { sortTableData("CreatorID") }} className="flex cursor-pointer justify-center text-orange-600">Creator User ID</div>),
                                    'itemquantity': 'Quantity',
                                    'timestamp': (<div onClick={() => { sortTableData("Time") }} className="flex cursor-pointer justify-center text-orange-600">Time</div>),
                                    'type': ((<div onClick={() => { sortTableData("Type") }} className="flex cursor-pointer justify-center text-orange-600">Type</div>)),
                                    'print': ((<div className="flex cursor-pointer justify-center text-orange-600">Print</div>)),
                                    orderstatus: (<div onClick={() => { sortTableData("Status") }} className="flex cursor-pointer justify-center text-orange-600">Status</div>),
