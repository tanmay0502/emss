...
export default function VerificationList() {
    ...
    const [Physical_List, setPhysical_List] = useState([]);
    ...
    async function List_Of_PhysicalVerification() {
        try {
            const response = await fetch(
                `${process.env.REACT_APP_API_SERVER}/unit/listEPV`,
                {
                    method: "GET",
            ...
            const verification_list = await response.json();
            console.log(verification_list, 'verification')
            if (response.status == 200)
                setPhysical_List(verification_list['data']);
            else
                alert(verification_list['message'])
            ...
            {isDetail === 0 ?
                <div class={styles.table}>
                    ...
                    <DynamicDataTable
                        ...
                        buttons={[]}
                        onClick={(event, row) => {
                            navigate("/session/unitmanagement/edit_verification/" + row["orderid"])
                        }}
                    />
                </div>
                : ''
