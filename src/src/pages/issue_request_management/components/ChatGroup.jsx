        ...
        {   
            groupedData && groupedData.map((elem) => {
                if(elem[0]['autogenerated'] === 'Y'){
                    return <ChatUpdate message={elem} docData={props.data ? props.data['supportingDocuments'] : {}} />
                }
                if ( elem[0]['remarkeruserid'] === window.sessionStorage.getItem('sessionToken') ){
                    return <SentMessageGroup data={elem} />
                }
                return <RecievedMessageGroup data={elem} />
            })
        }
    </>
  )
